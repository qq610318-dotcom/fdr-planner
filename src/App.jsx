...
const B = {
  ...
  bg: "#fdf6f2", card: "#ffffff", card2: "#fff4f0",
};
...
export default function App() {
  ...
  const inp = (extra = {}) => ({
    background: "rgba(180,130,120,.12)", border: "1px solid rgba(201,164,154,.2)",
    borderRadius: 8, padding: "9px 12px", color: "#3a2420", fontSize: 13,
  ...
  if (screen === "login") return (
    <div style={{ minHeight: "100vh", background: B.bg, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24 }}>
      ...
      <div style={{ position: "relative", zIndex: 1, width: "100%", maxWidth: 340 }}>
        ...
        <div style={{ background: "#ffffff", borderRadius: 18, padding: 24, boxShadow: "0 4px 24px rgba(180,120,110,.12)", border: `1px solid ${B.rose}20` }}>
          <div style={{ fontSize: 13, color: "#8a6560", marginBottom: 8, fontWeight: 600 }}>你的名字</div>
          ...
          <div onClick={() => setIsLeader(v => !v)} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: isLeader ? 12 : 20, cursor: "pointer", userSelect: "none" }}>
            ...
            <span style={{ fontSize: 13, color: "#8a6560" }}>我是團隊長</span>
          </div>
        ...
        <div style={{ textAlign: "center", fontSize: 11, color: "#b09a96", marginTop: 16 }}>進度資料會同步給團隊長查看</div>
      </div>
    </div>
  );
  ...
  if (screen === "leader") {
    ...
    return (
      <div style={{ minHeight: "100vh", background: B.bg, color: "#3a2420", fontFamily: "inherit", maxWidth: 480, margin: "0 auto" }}>
        <div style={{ position: "fixed", inset: 0, maxWidth: 480, margin: "0 auto", background: "radial-gradient(ellipse at 50% 0%,rgba(201,164,154,.15) 0%,transparent 60%)", pointerEvents: "none", zIndex: 0 }} />
        <div style={{ position: "relative", zIndex: 1, padding: "22px 20px 0" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
            ...
            <button onClick={() => { setScreen("login"); setMyName(""); }} style={{ background: "rgba(180,130,120,.10)", border: "1px solid rgba(255,255,255,.08)", borderRadius: 8, padding: "5px 12px", color: "#9a7a76", fontSize: 11, cursor: "pointer" }}>登出</button>
          </div>
        ...
        <div style={{ position: "relative", zIndex: 1, padding: "14px 20px 100px" }}>
          {tab === "week" && (
            <div>
              <div style={{ fontSize: 12, color: "#9a7a76", marginBottom: 14 }}>修改後所有夥伴即時看到更新 ✨</div>
              {teamSchedule.map((s, i) => (
                <div key={s.day} style={{ background: `${s.phaseColor}0d`, border: `1px solid ${s.phaseColor}25`, borderRadius: 14, padding: "13px 14px", marginBottom: 10, display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 13, fontWeight: 700, color: "#3a2420", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{s.title}</div>
                    <div style={{ fontSize: 10, color: "#9a7a76", marginTop: 2 }}>{s.checklist.length} 項清單</div>
                  </div>
          ...
          {tab === "team" && (
            <div>
              {memberNames.length === 0 ? (
                <div style={{ textAlign: "center", padding: "50px 0", color: "#9a7a76" }}>
                ...
                <>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
                    <div style={{ background: `${B.rose}0d`, border: `1px solid ${B.rose}20`, borderRadius: 12, padding: "14px", textAlign: "center" }}>
                      ...
                      <div style={{ fontSize: 11, color: "#9a7a76" }}>活躍夥伴</div>
                    </div>
                    <div style={{ background: "rgba(168,213,176,.06)", border: "1px solid rgba(168,213,176,.15)", borderRadius: 12, padding: "14px", textAlign: "center" }}>
                      ...
                      <div style={{ fontSize: 11, color: "#9a7a76" }}>進度優秀</div>
                    </div>
                  </div>
                  {memberNames.map(name => {
                    ...
                    return (
                      <div key={name} style={{ background: B.card2, border: "1px solid rgba(180,130,120,.18)", borderRadius: 14, padding: "14px 16px", marginBottom: 12 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 11, marginBottom: 12 }}>
                          <Avatar name={name} size={36} />
                          <div style={{ flex: 1 }}>
                            <div style={{ fontWeight: 700, fontSize: 14 }}>{name}</div>
                            <div style={{ fontSize: 11, color: "#9a7a76", marginTop: 1 }}>{doneDays}/7 天完成 · 整體 {overallPct}%</div>
                          </div>
                          ...
                          {DAY_LABELS.map(d => {
                            ...
                            return (
                              <div key={d} style={{ textAlign: "center" }}>
                                ...
                                <div style={{ fontSize: 9, color: "#b09a96", marginTop: 2 }}>{d.slice(1)}</div>
                              </div>
                            );
                          })}
                        </div>
                        {DAY_LABELS.map(d => {
                          ...
                          return (
                            <div key={d} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 5 }}>
                              <div style={{ fontSize: 11, color: "#9a7a76", width: 22, flexShrink: 0 }}>{d.slice(1)}</div>
                              <div style={{ flex: 1, height: 4, background: "rgba(180,130,120,.12)", borderRadius: 2 }}>
        ...
        {editing && editDayIdx !== null && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(80,40,36,.5)", zIndex: 100, display: "flex", alignItems: "flex-end", maxWidth: 480, margin: "0 auto" }} onClick={() => setEditing(false)}>
            <div style={{ width: "100%", background: "#ffffff", borderRadius: "22px 22px 0 0", padding: "20px 20px 36px", maxHeight: "90vh", overflowY: "auto", borderTop: `1px solid ${B.rose}30` }} onClick={e => e.stopPropagation()}>
              <div style={{ width: 36, height: 3, background: `${B.rose}60`, borderRadius: 2, margin: "0 auto 18px" }} />
              <div style={{ fontWeight: 800, fontSize: 16, marginBottom: 2 }}>✏️ 編輯 {teamSchedule[editDayIdx]?.day}</div>
              <div style={{ fontSize: 12, color: "#9a7a76", marginBottom: 18 }}>儲存後所有夥伴立即看到更新</div>
              {[["標題","title","input"],["Emoji","icon","input"],["階段名稱","phase","input"],["策略提示","tip","ta3"]].map(([label,key,type]) => (
                <div key={key} style={{ marginBottom: 13 }}>
                  <div style={{ fontSize: 12, color: "#8a6560", marginBottom: 5, fontWeight: 600 }}>{label}</div>
              ...
              <div style={{ marginBottom: 13 }}>
                <div style={{ fontSize: 12, color: "#8a6560", marginBottom: 5, fontWeight: 600 }}>建議素材 <span style={{ fontWeight: 400, color: "#9a7a76" }}>(每行一項)</span></div>
              ...
              <div style={{ marginBottom: 22 }}>
                <div style={{ fontSize: 12, color: "#8a6560", marginBottom: 5, fontWeight: 600 }}>執行清單 <span style={{ fontWeight: 400, color: "#9a7a76" }}>(每行一項)</span></div>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => setEditing(false)} style={{ flex: 1, background: "rgba(180,130,120,.10)", border: "1px solid rgba(255,255,255,.08)", borderRadius: 12, padding: "13px", color: "#8a6560", fontSize: 14, cursor: "pointer" }}>取消</button>
  ...
  return (
    <div style={{ minHeight: "100vh", background: B.bg, color: "#3a2420", fontFamily: "inherit", maxWidth: 480, margin: "0 auto" }}>
      <div style={{ position: "fixed", inset: 0, maxWidth: 480, margin: "0 auto", background: "radial-gradient(ellipse at 50% 0%,rgba(201,164,154,.15) 0%,transparent 60%),radial-gradient(ellipse at 80% 90%,rgba(160,120,144,.06) 0%,transparent 55%)", pointerEvents: "none", zIndex: 0 }} />
      <div style={{ position: "relative", zIndex: 1, padding: "22px 20px 0" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 11 }}>
            <FDRLogo size={46} />
          ...
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 26, fontWeight: 900, color: B.rose, lineHeight: 1 }}>{totalMyDone}<span style={{ fontSize: 11, color: "#9a7a76", fontWeight: 400 }}>/{DAY_LABELS.length}</span></div>
            <div style={{ fontSize: 10, color: "#9a7a76", marginBottom: 4 }}>本週完成</div>
      ...
      <div style={{ position: "relative", zIndex: 1, padding: "14px 20px 100px" }}>
        {tab === "week" && (
          <div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
              <button onClick={() => setWeekOffset(o=>o-1)} style={{ background: "none", border: "none", color: "#8a6560", fontSize: 22, cursor: "pointer", padding: "0 8px" }}>‹</button>
              <span style={{ fontSize: 12, color: "#8a6560" }}>
                {weekOffset === 0 ? "本週" : weekOffset > 0 ? `+${weekOffset} 週` : `${weekOffset} 週`}
                <span style={{ color: "#b09a96", fontSize: 11, marginLeft: 6 }}>
              ...
              {DAY_LABELS.map((dayLabel, i) => {
                ...
                return (
                  <div key={dayLabel} style={{ background: isToday ? `${B.rose}10` : "rgba(255,255,255,.02)", border: isToday ? `1px solid ${B.rose}38` : "1px solid rgba(255,255,255,.05)", borderRadius: 14, overflow: "hidden" }}>
                    <div style={{ display: "flex", alignItems: "center", padding: "11px 14px", gap: 11 }}>
                      ...
                      {sched ? (
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ fontSize: 13, fontWeight: 700, color: "#3a2420", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{sched.icon} {sched.title}</div>
                        </div>
                      ) : <div style={{ flex: 1, fontSize: 12, color: "#b09a96" }}>休息日</div>}
                      {sched && (
                    ...
                    {sched && prog > 0 && (
                      <div style={{ height: 2, background: "rgba(180,130,120,.10)" }}>
        ...
        {tab === "progress" && (
          <div>
            <div style={{ fontSize: 12, color: "#9a7a76", marginBottom: 14 }}>點擊卡片更新執行清單 ✨</div>
                ...
                <div key={sched.day} onClick={() => setSelected(sched.day)} style={{ background: `${sched.phaseColor}0f`, border: `1px solid ${sched.phaseColor}25`, borderRadius: 14, padding: "13px 16px", marginBottom: 10, cursor: "pointer", display: "flex", gap: 13, alignItems: "center" }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 3 }}>
                      <span style={{ fontSize: 10, color: "#9a7a76" }}>{sched.day}</span>
                      <span style={{ fontSize: 10, color: sched.phaseColor, fontWeight: 600 }}>{sched.phase}</span>
                    </div>
                    <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 6, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", color: "#3a2420" }}>{sched.title}</div>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
                      <span style={{ fontSize: 10, color: "#9a7a76" }}>{checks.filter(Boolean).length}/{sched.checklist.length} 項</span>
          ...
          <div>
            <div style={{ background: `${B.rose}0f`, border: `1px solid ${B.rose}28`, borderRadius: 16, padding: "16px", marginBottom: 16, display: "flex", gap: 12, alignItems: "center" }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 11, color: "#8a6560", lineHeight: 1.9 }}>週一播種 → 週二升溫 → 週三建信任<br/>週四推決策 → 週五收割 → 週六強化 → 週日留白</div>
              </div>
            </div>
            {teamSchedule.map(sched => (
              <div key={sched.day} style={{ background: `${sched.phaseColor}0f`, border: `1px solid ${sched.phaseColor}25`, borderRadius: 14, padding: "14px 16px", marginBottom: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 10, color: "#9a7a76", marginTop: 1 }}>{sched.phase}</div>
                  </div>
                </div>
                {sched.tip && (
                  <div style={{ background: "rgba(180,130,120,.10)", borderRadius: 10, padding: "10px 12px", marginBottom: 8, borderLeft: `2px solid ${sched.phaseColor}` }}>
                    <div style={{ fontSize: 10, color: "#9a7a76", marginBottom: 3 }}>💡 策略提示</div>
                    <div style={{ fontSize: 12, color: "#c8b0aa", lineHeight: 1.7 }}>{sched.tip}</div>
                  </div>
                )}
                <div style={{ fontSize: 11, color: "#9a7a76" }}><span style={{ color: "#8a6560", fontWeight: 600 }}>素材：</span>{sched.materials.join(" · ")}</div>
              </div>
            ))}
          </div>
        )}
      ...
      {selected && (() => {
        ...
        return (
          <div style={{ position: "fixed", inset: 0, background: "rgba(80,40,36,.5)", zIndex: 100, display: "flex", alignItems: "flex-end", maxWidth: 480, margin: "0 auto" }} onClick={() => setSelected(null)}>
            <div style={{ width: "100%", background: "#ffffff", borderRadius: "22px 22px 0 0", padding: "20px 20px 36px", maxHeight: "90vh", overflowY: "auto", borderTop: `1px solid ${B.rose}28` }} onClick={e => e.stopPropagation()}>
              <div style={{ width: 36, height: 3, background: `${B.rose}60`, borderRadius: 2, margin: "0 auto 18px" }} />
              ...
              <div style={{ display: "flex", gap: 12, marginBottom: 12 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 18, fontWeight: 900, color: "#3a2420", lineHeight: 1.2, marginBottom: 5 }}>{sched.title}</div>
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    ...
                    <span style={{ fontSize: 11, color: "#9a7a76", background: "rgba(180,130,120,.10)", borderRadius: 6, padding: "2px 8px" }}>📅 {selected}</span>
                  </div>
                </div>
                <div style={{ fontSize: 18, fontWeight: 900, color: prog === 100 ? "#a8d5b0" : sched.phaseColor }}>{prog === 100 ? "✅" : `${prog}%`}</div>
              </div>
              {sched.tip && (
                <div style={{ background: "rgba(180,130,120,.12)", borderRadius: 12, padding: "12px 14px", marginBottom: 14, borderLeft: `3px solid ${sched.phaseColor}` }}>
                  <div style={{ fontSize: 10, color: "#9a7a76", marginBottom: 4 }}>💡 今日策略提示</div>
                  <div style={{ fontSize: 13, color: "#c8b0aa", lineHeight: 1.7 }}>{sched.tip}</div>
                </div>
              )}
              {sched.materials.length > 0 && (
                <div style={{ marginBottom: 16 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: "#8a6560", marginBottom: 8 }}>📸 建議素材</div>
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    {sched.materials.map((m,i) => <span key={i} style={{ fontSize: 11, background: "rgba(180,130,120,.10)", border: "1px solid rgba(255,255,255,.07)", borderRadius: 8, padding: "4px 10px", color: "#8a6560" }}>{m}</span>)}
                  </div>
                </div>
              )}
              <div style={{ marginBottom: 20 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: "#8a6560" }}>✅ 執行清單</div>
                ...
                {sched.checklist.map((item, i) => (
                  <div key={i} onClick={() => toggleCheck(selected, i)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 0", borderBottom: "1px solid rgba(255,255,255,.04)", cursor: "pointer" }}>
                    <div style={{ width: 22, height: 22, borderRadius: 6, border: `2px solid ${checks[i] ? sched.phaseColor : "#d4b8b4"}`, background: checks[i] ? sched.phaseColor : "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "all .2s" }}>
